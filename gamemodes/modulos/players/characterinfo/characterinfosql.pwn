...
public PrepareCharacterInfoTable() {
    new query[255];
    mysql_format(mysql,query,255,"CREATE TABLE IF NOT EXISTS characterinfo (username VARCHAR(64) NOT NULL PRIMARY KEY,interiorid INT, skin INT,x FLOAT,y FLOAT,z FLOAT,hp FLOAT,armor FLOAT,wanted INT,money INT,virtualworld INT,GENDER INT);");
...
public FinishLoadCharacterInfo(playerid) {
    if(cache_num_rows()) {
        ...
        cache_get_value_index_int(0,11,gCharacterInfo[playerid][PLAYERINFO_GENDER]);
    ...
    mysql_format(mysql,query,255,"INSERT INTO characterinfo (username,interiorid, skin, x, y, z, hp, armor, wanted, money, virtualworld,gender)\
    VALUES ('%s',%d, %d, %f, %f, %f, %f, %f, %d, %d, %d,%d)",GetPlayerNameEx(playerid),GetPlayerInterior(playerid),GetPlayerSkin(playerid),x,y,z,hp,armor,GetPlayerWantedLevel(playerid),GetPlayerMoney(playerid),GetPlayerVirtualWorld(playerid),0);
...
public PrepareSaveCharacterInfo(playerid) {
    new query[255];
    new Float:x,Float:y,Float:z,Float:hp,Float:armor;
    new skin,wanted,money,virtualworld,gender;
    ...
    GetPlayerArmour(playerid,armor);
    GetPlayerPos(playerid,x,y,z);
    mysql_format(mysql,query,255,"UPDATE characterinfo SET interiorid = %d, skin = %d, x = %f, y = %f, z = %f, hp = %f, armor = %f, wanted = %d, money = %d, virtualworld = %d,gender = %d WHERE username = '%s'",GetPlayerInterior(playerid),skin,x,y,z,hp,armor,wanted,money,virtualworld,GetPlayerNameEx(playerid),gender);
