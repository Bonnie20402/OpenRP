...
forward PlayerInvDropInit();
public PlayerInvDropInit() {
...
forward GetPlayerInvDropModelid(index);
public GetPlayerInvDropModelid(index) {
...
forward GetEmptyPlayerInvDropIndex();
public GetEmptyPlayerInvDropIndex() {
    for(new i;i<MAX_PICKUPS;i++) {
...
forward GetPlayerInvDropAreaId(index);
public GetPlayerInvDropAreaId(index) {
    for(new i;i<MAX_PICKUPS;i++) {
...
forward GetPlayerInvDropIndexEx(pickupid);
public GetPlayerInvDropIndexEx(pickupid) {
    for(new i;i<MAX_PICKUPS;i++) {
        if(IsValidPlayerInvDrop(i)&&gInv_control_droppedItens[i][INVDROP_PICKUPID] == pickupid)return i;
    }
    return INVENTORY_INVALID_DROP;
}
forward GetPlayerInvDropPickupId(sphereid);
public GetPlayerInvDropPickupId(sphereid) {
...
forward IsValidPlayerInvDrop(index);
public IsValidPlayerInvDrop(index) {
...
forward ClearPlayerInvDropData(index);
public ClearPlayerInvDropData(index) {
...
forward DropPlayerInvItem(playerid,modelid,quantity);
public DropPlayerInvItem(playerid,modelid,quantity) {
...
forward GetPlayerInvDropQuantity(index);
public GetPlayerInvDropQuantity(index) {
...
forward GetPlayerInvDropIndex(playerid);
public GetPlayerInvDropIndex(playerid) {
    for(new i;i<MAX_PICKUPS;i++) {
        if(IsValidPlayerInvDrop(i)) {
